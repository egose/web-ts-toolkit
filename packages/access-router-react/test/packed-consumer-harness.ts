 ...
 *
 * The packed-consumer compatibility test and the documentation compile test
 * both need to stage the access-router-react npm tarball through the real
 * `@repo-toolkit/publish-package` release transformation, install it (plus its
 * internal `@web-ts-toolkit/access-router-client` + `@web-ts-toolkit/utils`
 * dependency closure, plus the external React peer deps) into a fresh
 * external-consumer tree under `/tmp`, and run `tsc` against the installed
 * declarations via the export map (no `tsconfig.json` `paths` override).
 * Centralizing the staging + install plumbing here keeps the two test files
 * from drifting on the staging contract — mirroring the access-router-client
 * harness layout (ARC-18 + ARC-20).
 *
 * `preparePackedWorkspace()` is memoized per test process — both tests reuse
 * the same staged tarballs so we do not pay the pack/pack cost twice.
 */
...
// The React peer deps live in the access-router-react package's own
// devDependencies (not the root) — read them from there so the external
// consumer resolves the same React versions the workspace builds against.
const reactPackageManifest = JSON.parse(readFileSync(path.resolve(packageRoot, 'package.json'), 'utf8')) as {
  devDependencies: Record<string, string>;
};
...
export const internalDependencyPackages = [
  { name: '@web-ts-toolkit/access-router-client', dir: path.resolve(workspaceRoot, 'packages/access-router-client') },
...
/**
 * Stage and `pnpm pack` the access-router-react package and its internal
 * `@web-ts-toolkit/access-router-client` + `@web-ts-toolkit/utils` dependency
 * closure so the produced tarballs mirror exactly what
 * `repo-toolkit-publish-package` ships at release time. Result is memoized per
 * test process; both ARR-10 packed tests reuse the same staged tarballs
 * rather than paying the pack/pack cost twice.
 *
 * Re-run the package `build`(s) before invoking this so the staged `dist/`s
 * reflect the current source.
 */
export function preparePackedWorkspace(): PackedWorkspace {
  ...
  const tempRoot = trackTempRoot(mkdtempSync(path.join(os.tmpdir(), 'access-router-react-arr10-')));
...
/**
 * Install the staged React tarball + internal dependency closure + external
 * React peer deps into a fresh external consumer tree under `/tmp`. External
...
export function installPackedConsumer(): string {
  ...
  // release closure). External peer deps (`react`, `react-dom`) come from the
  // registry exactly how an external consumer resolves them.
  ...
  writeFileSync(
    ...
    `${JSON.stringify(
      {
        name: 'access-router-react-consumer',
        private: true,
        type: 'module',
        dependencies: {
          ...internalDeps,
 ...
 * returned directory is registered for teardown. The packed-consumer
 * compatibility test and the documentation compile test use this to
 * inspect the packed tree without polluting the shared stage.
 */
export function unpackTarballToDir(tarballPath: string): string {
  const unpackRoot = trackTempRoot(mkdtempSync(path.join(os.tmpdir(), 'access-router-react-arr10-unpack-')));
