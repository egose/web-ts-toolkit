...
Templates are trusted application code. The service does not accept caller-supplied template source, and template functions run with the host application's authority. Registering a template validates duplicate/ambiguous action metadata, then stores a frozen shallow snapshot of content and action structure so later accidental object mutation cannot change registered authorization, default-action selection, handlers, or rendered content. Registration takes a mutable `MessageTemplate`; `find()`/`getAll()` return readonly `RegisteredMessageTemplate` views (top-level, content objects, object uiTemplates, the action array, and each action plus its confirmation/payload are frozen — writes throw at runtime and fail strict type checking). Function fields keep their identity; replacing behavior requires registering a replacement template.
...
Content rendering is separated from action evaluation: creation renders only content from `PrepareResult.templateData` (`interpolateMessageContent`) and never runs `condition` predicates, so message-field conditions cannot fail creation. Eligible active `getActions`/`handleAction` evaluate predicates against the persisted message; admin/archived listings resolve only `uiTemplate` (`resolveUiTemplate`) with an empty list and skip conditions. Action labels/confirmations compile from persisted `message.payload`, distinct from content `templateData` (missing values render empty; no unified interpolation-data feature). The combined `interpolateTemplate` helper is preserved with the same escaping/trusted-template semantics.
...
Reservations have one explicit state machine: `pending` while a lease owner prepares work, `completed` after the full batch and `itemCount` commit atomically, and `failed` after creation fails. A `failed` reservation persists the raw diagnostic as `failureMessage`; replays raise `ClientRequestFailedError` with a stable public message (`…previously failed; retry with a new clientRequestId`) while the diagnostic stays on `error.failureReason`/`cause` and in the record. The HTTP boundary maps the failed replay to 409 with that stable message only — provider/template/database cause text never crosses into the response. Other mapped lifecycle errors were audited for the same crossing: pending/conflict forward only caller-supplied or authorized message/attempt ids, archived outcomes stay behind the sender/receiver relationship gate, and retryable/notification causes remain on `error.cause`, which serializers do not emit. Replays are allowed only from `completed` reservations whose messages contain exactly the expected distinct item indexes merged across the active and archive collections (current-records contract: `Array<IMessage | IMessageArchive>` sorted by item index, archive entries unmodified with no fabricated active methods). Reads run active-then-archive — archival's create-then-delete order means a moving item is always visible in at least one snapshot — deduplicate by `_id`, reconcile once before declaring corruption, and re-read a missing reservation before declaring orphan messages, so concurrent commits and archive movement cannot false-corrupt. Pending reservations have bounded waits and an expiring lease so a crashed owner does not block retries indefinitely. Archive retention bounds the replay window: deleted archive documents surface as `ClientRequestInconsistentStateError`.

Lease/wait/poll durations are finite safe integers validated once at construction (`InvalidMessageServiceOptionError`): lease and poll in `[1, 2147483647]` ms, wait in `[0, 2147483647]` ms with `0` as a supported immediate check. The ceiling is Node's maximum `setTimeout` delay (`2^31 - 1`, exported as `MAX_MESSAGE_SERVICE_TIMEOUT_MS`); `NaN`/infinite/fractional/negative (and zero lease/poll)/overflow values fail fast instead of producing no-deadline waits or aggressive polling. One clock seam (`clientRequestNow`, default `Date.now`) drives both the elapsed-time duplicate-wait deadline and the persisted wall-clock `leaseExpiresAt` (`new Date(now())`), with the stale-takeover filter compared against the same clock; the polling delay seam (`clientRequestDelay`, default `setTimeout`) keeps boundary tests deterministic without long sleeps. The wait deadline bounds polling only and never cancels a hung database/provider operation — each reservation/message read is awaited, then the deadline decides between another poll and `ClientRequestPendingError`.
...
Payment sessions are external side effects created before the message commit point. If any prepared item, model/session setup step, or the idempotent batch transaction fails before commit, the service compensates by calling the provider's idempotent `expireSession()` (bound to the provider instance) for every newly created uncommitted session tracked from preparation through the commit attempt — including earlier batch items when a later item's provider/render step fails, and sessions stranded by model resolution, `startSession()`, persistence, or rollback failures. Every known session is attempted even when an earlier expiration or the observer fails: `onPaymentCompensationFailure` is invoked once per failed session with the session, scope, original error, and cleanup error, and the create call fails with `PaymentSessionCompensationError` for a lone session or `PaymentSessionCompensationAggregateError` (a subclass carrying every per-session failure in `failures` with the triggering error as `originalError`) for multi-session batches. Committed sessions are never expired because post-commit housekeeping failed, and non-idempotent sequential creation keeps per-item semantics (only the failing item is compensated; already-committed items are retained, never silently converted to all-or-nothing). Process death before a returned session is recorded, or any ambiguous provider/commit outcome the process cannot observe, must be reconciled with the provider out of band. A completed same-scope retry replays the committed messages and does not create replacement sessions.
