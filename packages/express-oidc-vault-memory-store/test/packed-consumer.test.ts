...
const memoryStorePackageName = '@web-ts-toolkit/express-oidc-vault-memory-store';
...
const packageSpecs: PackageSpec[] = [
  ...
  {
    name: memoryStorePackageName,
    root: packageRoot,
    directory: 'packages/express-oidc-vault-memory-store',
    tarballName: `web-ts-toolkit-express-oidc-vault-memory-store-${testVersion}.tgz`,
  },
];
...
function preparePackedWorkspace(): PackedWorkspace {
  ...
  const tempRoot = trackTempRoot(mkdtempSync(path.join(os.tmpdir(), 'express-oidc-vault-memory-store-mem06-')));
...
function unpackTarballToDir(tarballPath: string): string {
  const unpackRoot = trackTempRoot(
    mkdtempSync(path.join(os.tmpdir(), 'express-oidc-vault-memory-store-mem06-unpack-')),
  );
...
function installPackedConsumer(): string {
  ...
  const memoryStoreSourcePackageJson = sourcePackageJsons[memoryStorePackageName];
  const coreSourcePackageJson = sourcePackageJsons[corePackageName];
  const consumerDir = trackTempRoot(mkdtempSync(path.join(os.tmpdir(), 'express-oidc-vault-memory-store-consumer-')));
  seedToolVersions(consumerDir);
  writeFileSync(
    path.resolve(consumerDir, 'package.json'),
    `${JSON.stringify(
      {
        name: 'express-oidc-vault-memory-store-consumer',
        private: true,
        type: 'module',
        dependencies: {
          [corePackageName]: `file:${packed.tarballs[corePackageName]}`,
          [memoryStorePackageName]: `file:${packed.tarballs[memoryStorePackageName]}`,
          express: coreSourcePackageJson.devDependencies?.express,
        },
        devDependencies: {
          '@types/express': coreSourcePackageJson.devDependencies?.['@types/express'],
          '@types/node': memoryStoreSourcePackageJson.devDependencies?.['@types/node'],
...
describe('MEM-06 packed-package consumer compatibility', () => {
  it('applies the real publish manifest transformation to the memory-store tarball', () => {
    const packed = preparePackedWorkspace();
    const stagedManifest = packed.manifests[memoryStorePackageName];
    const unpackRoot = unpackTarballToDir(packed.tarballs[memoryStorePackageName]);
  ...
  it('`npm pack --dry-run --json` lists only intended files in the staged memory-store tree', () => {
    const packed = preparePackedWorkspace();
    const stageDir = path.resolve(packed.tempRoot, memoryStorePackageName.replace(/[@/]/g, '_'));
  ...
  it('installs staged tarballs and runs CJS, ESM, NodeNext, Bundler, and README consumers', () => {
    ...
    const installedPackageDir = path.resolve(
      ...
      'node_modules',
      '@web-ts-toolkit',
      'express-oidc-vault-memory-store',
    );
