...
/**
 * ARC-18: Test the packed CJS, ESM, and declaration surface.
 *
 ...
 * release time — to stage and pack the package, install the staged tarball
 * plus its internal `@web-ts-toolkit/utils` dependency closure in a fresh
 * consumer, then execute CJS `require`, ESM `import`, NodeNext typecheck, and
 * Bundler typecheck with `strict: true` and `skipLibCheck: false`. Manifest
 * and packed-file assertions prove version/license/repository rewriting,
 * `workspace:*` resolution, and the deliberate ESM/CJS declaration-condition
 * mapping (so the emitted `.d.mts` is reachable rather than shipped as
 * unreachable dead weight).
 *
 * The install/staging plumbing lives in `./packed-consumer-harness.ts` and
 * is shared with the ARC-20 documentation compile test so the two files do
...
describe('ARC-18 packed-package compatibility using the real release transformation', () => {
  it('applies the real `@repo-toolkit/publish-package` manifest transformation to the access-router-client tarball', () => {
    const packed = preparePackedWorkspace();
    const stagedManifest = packed.manifests[clientPackage.name];
    const unpackRoot = unpackTarballToDir(packed.tarballs[clientPackage.name]);
    ...
    // `.d.ts`, so the emitted `.d.mts` is reachable (requirement #4 of
    // ARC-18: do not ship an unreachable `.d.mts` without rationale).
  ...
  it('`npm pack --dry-run --json` lists only intended files in the staged access-router-client tree', () => {
    const packed = preparePackedWorkspace();
    const stageDir = path.resolve(packed.tempRoot, clientPackage.name.replace(/[@/]/g, '_'));
    ...
    // LICENSE + README.md + llms.txt + four dist outputs + package.json.
    const expectedFiles = [
      'LICENSE',
      'README.md',
      'index.d.mts',
  ...
  it('installs the staged tarball + internal dependency closure and runs CJS, ESM, NodeNext, and Bundler consumers', () => {
    ...
    // export surface (matched against ARC-17's curated contract).
    ...
    // Sanity: the installed consumer has a real `@web-ts-toolkit/access-router-client`
    // in its node_modules (no workspace path mapping, no source symlink).
    const installedClientDir = path.resolve(consumerDir, 'node_modules', '@web-ts-toolkit', 'access-router-client');
    expect(existsSync(path.resolve(installedClientDir, 'package.json'))).toBe(true);
    const installedManifest = JSON.parse(
      readFileSync(path.resolve(installedClientDir, 'package.json'), 'utf8'),
    ) as PackageJson;
    expect(installedManifest.version).toBe(testVersion);
    for (const emitted of ['index.js', 'index.mjs', 'index.d.ts', 'index.d.mts']) {
      expect(existsSync(path.resolve(installedClientDir, emitted))).toBe(true);
    }
    ...
    const allFiles = walk(installedClientDir).map((p) => path.relative(installedClientDir, p).replace(/\\/g, '/'));
