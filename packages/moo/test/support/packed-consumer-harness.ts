...
export const testVersion = '0.99.0-moo-moo09';
...
export const workspacePackages = [
  ...
  { name: '@web-ts-toolkit/moo', dir: packageRoot },
] as const;

/** Every subpath in packages/moo/package.json "exports". */
export const mooSubpaths = [
  '.',
  './is',
  './schema',
  './utils',
  './plugins',
  './plugins/cascade-delete',
  './plugins/model-function',
  './plugins/new-document',
  './plugins/keycloak-user-sync',
] as const;
...
export function run(command: string, args: string[], cwd: string): string {
  ...
  // Vitest injects NODE_PATH entries pointing at the workspace pnpm store.
  // A packed consumer must resolve exactly like a real isolated install, so
  // strip it; the consumer's own node_modules (via cwd) still resolve.
  delete env.NODE_PATH;
  try {
    return execFileSync(command, args, { cwd, encoding: 'utf8', stdio: 'pipe', env });
  } catch (error) {
...
export function preparePackedWorkspace() {
  ...
  const tempRoot = mkdtempSync(path.join(os.tmpdir(), 'moo-moo09-packed-'));
...
export interface PackedConsumerOptions {
  /** Mongoose range for the isolated consumer (e.g. '^8.24.0' or '^9.8.0'). */
  mongooseVersion: string;
  /** Keycloak-fluent version for the consumer; omit for a core-only install without the optional peer. */
  keycloakVersion?: string;
}

export function installPackedConsumer(options: PackedConsumerOptions): string {
  const packed = preparePackedWorkspace();
  const label = `moo09-${options.keycloakVersion ? 'kc' : 'core'}-m${options.mongooseVersion.replace(/[^0-9]/g, '')}`;
  const consumerDir = mkdtempSync(path.join(os.tmpdir(), `${label}-consumer-`));
    ...
    `${JSON.stringify(
      {
        ...
        dependencies: {
          ...
          mongoose: options.mongooseVersion,
          ...(options.keycloakVersion ? { '@egose/keycloak-fluent': options.keycloakVersion } : {}),
        },
        devDependencies: {
