...
`RootRouter` accepts a batch array where each entry follows the same broad conventions as the dedicated public routes:

- top-level identifiers stay top-level: `model`, `op`, `id`, `field`, `filter`, `data`
...
- `create`, `list`, `read`, and `update` forward both `args` and `options` to the corresponding public service methods
