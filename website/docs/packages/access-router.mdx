...
Routers support runtime updates for request-time behavior such as hooks, filters, permissions, and defaults.

Build-time route-shape options are intentionally immutable after construction.

Examples of build-time options:

- model router: `basePath`, `parentPath`, `idParam`, `queryRouteSegment`, `mutationRouteSegment`
- data router: `basePath`, `parentPath`, `idParam`, `queryRouteSegment`

This means code like this is rejected after the router has already been created:
...
Use router construction for route shape, and use setters only for behavior that is meant to vary after construction.
...
The `advanced` subpath exports the same validation helpers used by the built-in public routers:
...
The main package entry keeps the common router/runtime API easy to discover.

For lower-level extension points, prefer the `advanced` subpath:

```ts
import { parseBody, requestSchemas, MIDDLEWARE, Codes } from '@web-ts-toolkit/access-router/advanced';
```

Use `@web-ts-toolkit/access-router/advanced` when you specifically need low-level items such as:

- validation helpers and schemas
- low-level symbols
- enum exports
- deeper TypeScript contract types

The main entrypoint stays focused on the common router/runtime API, while the `advanced` subpath is the intended import location for low-level usage.
