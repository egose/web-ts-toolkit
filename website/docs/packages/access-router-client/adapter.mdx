...
- `cacheCapacity?: number` — bounds the number of cached entries per adapter (LRU eviction)
...
When every member of a group explicitly passes `throwOnError: true` in its `axiosRequestConfig` (or the owning service was constructed with `throwOnError: true`), the **entire batch** rejects with the first `ServiceError` produced by any failing entry — the remaining entries are not finalized. This mirrors how a single `throwOnError: true` direct call rejects instead of returning a `{ success: false }` payload.
...
A batch is uniform: a group with mixed `throwOnError` settings does not form a coherent batch (the per-call `throwOnError` flag travels in the request metadata, and `group(...)` enforces one shared `AxiosRequestConfig` per batch, so members either all opt in or all opt out).
...
- batching metadata (`__op`, `__query`, `__requestConfig`, `__service`) is installed non-enumerably and non-writable, so it cannot accidentally leak through `JSON.stringify`, `Object.keys`, spread iteration, or consumer reassignment; direct property reads inside the client package (e.g. by `group(...)`) still work
