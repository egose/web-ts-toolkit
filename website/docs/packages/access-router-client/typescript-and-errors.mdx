...
A known field accepts its scalar, an array of those scalars (the sibling server expands it to an `$in` query), `RegExp` when the field is string-typed, and the comparison / element / evaluation operators valid for that scalar. Root operators (`$and` / `$nor` / `$or` / `$text` / `$where` / `$comment`) are typed only at the root of a filter. Unknown field keys and unknown operators do not compile:
...
- `Response<TRaw, TData = TRaw>` — discriminated union of `SuccessResult<TRaw, TData>` and `FailureResult<TRaw>`
