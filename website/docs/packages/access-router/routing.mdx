...
Root entries accept the same `include` shapes as direct routes: legacy
`localField`/`foreignField` joins and correlated entries
(`mode: 'correlated'` with `{ "$parent": "<field>" }` markers resolved per
